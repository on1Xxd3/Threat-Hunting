...
##Цель работы 1. Изучить возможности технологии Yandex Query для анализа
...
![](images/7.1.png) \### 2. Подключить бакет как источник данных для
Yandex Query

#### 2.1-2.2 Создать соединение для бакета в S3 хранилище и заполнить поля.

![](images/7.2.png) \#### 2.3-2.4 Создать привязку данных
![](images/7.3.png) ####Настроить формат данных для созданной привязки:
![](images/7.4.png) \#### 2.5 Проверка подключения
...
строки на список строк с разделителем `.`, применим функцию к `dst` и к
`src` и выберем количество уникальных адресов.
...
![](images/7.7.png) \#### 3.3. Определите суммарный объем входящего
трафика
...
Таким образом, с использованием сервиса Yandex Query был настроен доступ
к данным, размещённым в Yandex Object Storage, что позволило на практике
закрепить навыки применения SQL для анализа данных сетевой активности в
корпоративной сети.
